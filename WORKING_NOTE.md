...
The next batch, queued for the following daily quota window, was revised mid-preparation after that batch's results came in, specifically to avoid the two now-confirmed-risky axes (untested pool structures, calibration-confidence reduction) rather than continuing to extend them:
...
None of these are expected, with real confidence, to beat 92.540 — the honest goal for this batch is minimizing the chance of another large regression while the project's evidence base on genuinely positive new levers stays thin, not claiming a predicted new record. This note will be updated with the resulting real scores and their interpretation once they land, per the standing practice described at the top of this document.
