...
The previous batch (forge7, forge8, calibration-cut, and their combination) has fully landed and is now folded into Section 2.3's table and Section 2.4's calibration-promotion-risk finding above — all four results were flat-to-negative, none beat the standing 92.540 best.

The next batch was pushed 2026-08-20 (~00:05-00:09 UTC / ~05:35-05:39 IST), revised mid-preparation after that prior batch's results came in, specifically to avoid the two now-confirmed-risky axes (untested pool structures, calibration-confidence reduction) rather than continuing to extend them:
- Two isolated, pool-neutral and calibration-neutral single-lever tests carried over unchanged: a downward `TOP_HEAD_START` pull (300→150, an untested direction — prior dose-response data for this lever was measured on older, larger pools) and a generation-side fill-budget squeeze (`FILL_FRAC`/`MARGIN_S`, made safer by the mid-competition partial-credit-on-timeout change).
- A third pool-neutral, calibration-neutral single-lever test: a replay-side budget squeeze (`REPLAY_SAFE_FRAC`/`ENV_OVERHEAD_S`), same reasoning.
- The two remaining slots — originally planned as a forge6/7/8 pool-expansion variant and a forge6/7/8-plus-calibration-cut moonshot — were replaced rather than patched: a clean control resubmit of the standing-best pool (a second real anchor sample, since the first control resubmit this batch landed at 89.885, below the 92.540 record, suggesting the pool's true mean may sit closer to ~90–91 with 92.540 itself a favorable high roll), and a "safe moonshot" that only combines the three pool/calibration-neutral levers above, introducing no new failure surface.

None of these are expected, with real confidence, to beat 92.540 — the honest goal for this batch is minimizing the chance of another large regression while the project's evidence base on genuinely positive new levers stays thin, not claiming a predicted new record. All 5 confirmed pending as of the push (submission IDs 55633976, 55634002, 55634044, 55634068, 55634077). This note will be updated with the resulting real scores and their interpretation once they land, per the standing practice described at the top of this document.
