"""Seeded triplet runner: 3 seeds 42/123/999 via MockCompliantAgent, REAL_REPLAY_CEILING=150, wall-clock measured."""
import time, statistics, math, sys, json
from pathlib import Path
sys.path.insert(0, str(Path('comp_data').resolve()))
from tools.local_test import MockCompliantAgent, OptimalGuardrail, replay_and_score, predicted_real_score, load_attack, summarize
...
# compute t-test for pred vs null 55 (one-sample) ; also pairwise delta stats
...
# simple p approx via scipy if available
p_vs55 = None
try:
...
# also compute delta vs threshold 5 pts noise floor: mean delta vs 0? Use one-sample vs 5
try:
