...
Strict-review fixes vs v3/v4:
  ...
  F6) lean pool (12)        -> only field-proven forms + novel exfil+confused
      combos; calibration decides the winner per model.
...
def _b_post_deputy(urls, addrs, payload):
...
_STRUCTURES: tuple[dict[str, Any], ...] = (
    ...
    {"name": "post_deputy", "build": _b_post_deputy, "u": 1, "a": 1, "reps": CALIB_REPS,       "payload": PAYLOADS[0]},
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _search(self, env: Any, budget: float, max_hops: int) -> list[AttackCandidate]:
        ...
        def adaptive_margin() -> float:
            return min(self._margin_s, MARGIN_FLOOR_MIN + slowest * MARGIN_SLOWEST_COEF)

        def wall_ok() -> bool:
            reserve = max(adaptive_margin(), slowest * SLOWEST_MULT)
